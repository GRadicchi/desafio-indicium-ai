...
def generate_srag_charts(reference_date: str) -> Dict[str, str]:
    ...
    try:
        with engine.connect() as conn:
            # 1. Gráfico Semanal: Usando to_date para garantir cast seguro no Postgres
            query_weekly = text("""
                SELECT 
            ...
            # 2. Gráfico Mensal: Usando to_date e date_trunc para agrupamento eficiente
            query_monthly = text("""
                SELECT 
        ...
        # Retorna o erro estruturado para que a API possa lançar o HTTP 500 correto
        return {"error": str(e)}